...
class SVRTabView:
    ...
    def __create_layout(self):
        ...
        for label, entry in zip(self.__feature_entry_labels, self.__feature_entries):
            label.grid(row=self.__row_index, column=column_index, padx=10)
            entry.grid(row=self.__row_index + 1, column=column_index, padx=10, pady=(0, 10))
            column_index += 1
            if column_index == 3:
                column_index = 0
                self.__row_index += 2
    ...
    def __plot(self):
        self.__tab_view.grid_rowconfigure(self.__row_index + 1, weight=1)
        figure = plt.Figure()
        ...
        canvas.draw()
        canvas.get_tk_widget().grid(row=self.__row_index + 1, column=0, columnspan=3, sticky='NSEW')
    ...
    def predict(self):
        if not self.__dataset.empty:
            ...
            predicted_value = round(self.__y_standard_scalar.inverse_transform([predicted_value])[0][0], 3)

            self.__predicted_value_label.configure(text=f'Predicted {self.__predictable_column}: ₹{predicted_value}', font=customtkinter.CTkFont(size=20, weight="bold"))
            self.__predicted_value_label.grid(row=self.__row_index, column=0, columnspan=3, padx=10, pady=(0, 10), sticky='WE')
